...
function drawSnake(arr1, arr2, arr3, arr4) {
	...
	for (let i = 0; i < arr2.length; i++) {
		$(arr2[i].y + ">" + arr2[i].x).html('<img src="/Content/Images/apple.png" alt="A">');
	}
	for (let i = 0; i < arr3.length; i++) {
		$(arr3[i].y + ">" + arr3[i].x).html('<img src="/Content/Images/brick.png" alt="B">');
	}
	for (let i = 0; i < arr4.length; i++) {
		$(arr4[i].y + ">" + arr4[i].x).html('<img src="/Content/Images/gapple.png" alt="gA">');
	}
}
function clearBoard() {
...
let timer = setInterval(function() {
	...
	if (score % 5 == 4) {
		...
		$(apples[0].y + ">" + apples[0].x).html('<img src="/Content/Images/scroll.png" alt="S">');
	}
	addBorRads();
	info();
}, 1000/FPS);
